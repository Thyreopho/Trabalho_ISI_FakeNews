...
//#region user area
    ...
    //#region Global vars
        ...
        //#region election_set
            ...
            typedef struct count_fake {
                long int qtd;
                int yday;
            } count_fake;

            count_fake count_fake_per_day[50];
            int count_fake_per_day_p;
    ...
    void fillCountFakePerDay (FakeNews *line) {
        if (line->published) {
            int yday = localtime(&line->published)->tm_yday;
            bool foudFlag = false;
            for (int i = 0; i < count_fake_per_day_p && !foudFlag; i++) {
                if (count_fake_per_day[i].yday == yday) {
                    count_fake_per_day[i].qtd++;
                    foudFlag = true;
                }
            }

            if (!foudFlag) {
                count_fake_per_day[count_fake_per_day_p].yday = yday;
                count_fake_per_day[count_fake_per_day_p].qtd = 1;
                count_fake_per_day_p++;
            }
        }
    }
    ...
    void middleware (FakeNews *line) {
        ...
        if (filter(line)) {
            fillCountFakePerDay(line);
        }
    }

    void finally () {
        printf("\"dia_do_ano\",\"quantidade_de_noticias_falsas\"\n");
        for (int i = 0; i < count_fake_per_day_p; i++) {
            printf("%i,%li\n",
                count_fake_per_day[i].yday,
                count_fake_per_day[i].qtd
            );
        }
    }
